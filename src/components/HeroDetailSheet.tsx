        ...
        <div className="space-y-2">
          <div>
            <h2 className="font-heading text-3xl font-semibold text-foreground leading-tight tracking-wide">
              {heroName}{' '}
              <span className="inline-flex align-middle ml-1" title={gender}>
                {gender === 'Female' ? (
                  <Venus className="w-5 h-5 text-muted-foreground/50" />
                ) : (
                  <Mars className="w-5 h-5 text-muted-foreground/50" />
                )}
              </span>
            </h2>
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <span
              className={cn('text-xs sm:text-sm font-bold px-3 py-1.5 rounded-full', rarityStyle)}
            >
              {rarityName}
            </span>
            {hero.shiny && <Sparkles className="w-4 h-4 text-yellow-500 dark:text-yellow-400" />}
          </div>
          ...
          <div className="space-y-3">
            ...
            <div className="flex items-center justify-between">
              ...
              <span className="font-mono text-base font-medium text-foreground/80">
                ...
                {xpProgress.isMaxLevel ? 'MAX' : xpProgress.needed.toLocaleString()}
              </span>
            </div>
        ...
        <div>
          ...
          <div className="grid grid-cols-2 gap-x-6 gap-y-1.5">
            <StatBar
              ...
              value={hero.strength}
              max={statMax}
              labelColor={statColor('STR')}
              title={statTooltip('STR')}
            />
            <StatBar
              label="INT"
              value={hero.intelligence}
              max={statMax}
              labelColor={statColor('INT')}
              title={statTooltip('INT')}
            />
            <StatBar
              label="AGI"
              value={hero.agility}
              max={statMax}
              labelColor={statColor('AGI')}
              title={statTooltip('AGI')}
            />
            <StatBar
              label="WIS"
              value={hero.wisdom}
              max={statMax}
              labelColor={statColor('WIS')}
              title={statTooltip('WIS')}
            />
            <StatBar
              label="VIT"
              value={hero.vitality}
              max={statMax}
              labelColor={statColor('VIT')}
              title={statTooltip('VIT')}
            />
            <StatBar
              label="LCK"
              value={hero.luck}
              max={statMax}
              labelColor={statColor('LCK')}
              title={statTooltip('LCK')}
            />
            <StatBar
              label="END"
              value={hero.endurance}
              max={statMax}
              labelColor={statColor('END')}
              title={statTooltip('END')}
            />
            <StatBar
              label="DEX"
              value={hero.dexterity}
              max={statMax}
              labelColor={statColor('DEX')}
              title={statTooltip('DEX')}
            />
          </div>
          <div className="mt-3 pt-2 border-t border-border/15 flex items-center justify-between">
            <span className="text-sm text-muted-foreground/50 uppercase tracking-wide">
              Total Score
            </span>
      ...
      <div className="md:w-[55%] overflow-y-auto p-5 sm:p-6 space-y-6">
        ...
        <div>
          ...
          <div className="grid grid-cols-2 gap-x-6">
            {/* Primary column */}
            <div>
              <div className="text-xs text-muted-foreground/40 uppercase tracking-wider mb-1.5">
                Primary
              </div>
        ...
        <div>
          ...
          <GeneTable
            ...
            isFemale={visualGenes.gender.d === 3}
          />
        </div>
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      ...
      >
        <DialogHeader className="sr-only">
          <DialogTitle>{hero ? getHeroName(hero) : 'Hero Details'}</DialogTitle>
        </DialogHeader>
